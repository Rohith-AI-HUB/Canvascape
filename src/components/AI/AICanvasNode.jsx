...
// ─────────────────────────────────────────────────────────────────────────────
// Markdown renderer — full block + inline support
// Blocks : h1-h6, paragraphs, bullet lists (nested indent),
//          numbered lists, blockquotes, fenced code, horizontal rules
...
function renderInline(txt) {
  ...
  while ((m = pattern.exec(txt)) !== null) {
    ...
    if (f.startsWith('***'))
      ...
      parts.push(<strong key={m.index}>{m[3] || m[4]}</strong>)
    else if (f.startsWith('_') || f.startsWith('*'))
      parts.push(<em key={m.index} style={{ fontStyle:'italic' }}>{m[5] || m[6]}</em>)
    else if (f.startsWith('~~'))
      parts.push(<span key={m.index} style={{ textDecoration:'line-through', opacity:0.55 }}>{m[7]}</span>)
    else if (f.startsWith('`'))
      parts.push(<code key={m.index} style={{ fontFamily:"'DM Mono',monospace", fontSize:'0.875em', padding:'1px 6px', borderRadius:4, background:'rgba(245,158,11,0.13)', color:'var(--a)', border:'1px solid rgba(245,158,11,0.18)', userSelect:'text' }}>{m[8]}</code>)
    else if (f.startsWith('['))
      parts.push(<a key={m.index} href={m[10]} target="_blank" rel="noopener noreferrer" onClick={e=>e.stopPropagation()} style={{ color:'var(--a)', textDecoration:'underline', textUnderlineOffset:2, textDecorationThickness:1, cursor:'pointer' }}>{m[9]}</a>)
...
function tokenise(raw) {
  ...
  while (i < lines.length) {
    ...
    if (/^>\s?/.test(line)) {
      ...
      tokens.push({ type:'blockquote', lines:bq }); continue
    }
    ...
    if (/^(\s*)([-*+])\s+/.test(line)) {
      ...
      while (i < lines.length && /^(\s*)([-*+])\s+/.test(lines[i])) {
        ...
        items.push({ indent: Math.floor(lm[1].length / 2), text: lm[3] })
        i++
      }
    ...
    while (
      i < lines.length &&
      lines[i].trim() !== '' &&
      !/^(```|#{1,6}\s|>\s?|\s*[-*+]\s|\d+[.)]\s|(\s*[-*_]){3,})/.test(lines[i])
    ) { para.push(lines[i]); i++ }
    if (para.length) tokens.push({ type:'para', lines:para })
  }
  return tokens
}
...
function CodeBlock({ lang, body }) {
  ...
  return (
    <div style={{ borderRadius:10, overflow:'hidden', border:'1px solid rgba(255,245,220,0.08)', margin:'2px 0' }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'5px 11px', background:'rgba(0,0,0,0.42)', borderBottom:'1px solid rgba(255,255,255,0.05)' }}>
        <span style={{ fontSize:10, fontWeight:600, color:'rgba(255,245,220,0.4)', fontFamily:"'DM Mono',monospace", letterSpacing:'0.06em' }}>
          {lang || 'code'}
        </span>
        <button onMouseDown={e=>e.stopPropagation()} onClick={copy}
          style={{ display:'flex', alignItems:'center', gap:4, padding:'2px 9px', borderRadius:5, border:`1px solid ${copied?'rgba(52,211,153,0.3)':'rgba(255,255,255,0.09)'}`, background:copied?'rgba(52,211,153,0.1)':'rgba(255,255,255,0.05)', color:copied?'#34D399':'rgba(255,245,220,0.4)', fontSize:10, cursor:'pointer', fontFamily:'inherit', transition:'all 150ms' }}>
          {copied ? '✓ Copied' : 'Copy'}
        </button>
      </div>
      <pre style={{ margin:0, padding:'11px 13px', background:'rgba(10,9,6,0.6)', fontSize:11.5, fontFamily:"'DM Mono',monospace", color:'#E8DFC8', overflowX:'auto', lineHeight:1.65, userSelect:'text', cursor:'text', whiteSpace:'pre' }}>
        {body}
      </pre>
    </div>
  )
...
function MsgContent({ content }) {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:5, userSelect:'text' }}>
      {tokens.map((tok, idx) => {
        switch (tok.type) {

          case 'heading': {
            const sz  = [0,15.5,14.5,13.5,13,12.5,12][tok.level] || 13
            const fw  = [0,800, 750, 700, 680,660, 640][tok.level] || 680
            return (
              <div key={idx} style={{
                fontSize:sz, fontWeight:fw, color:'var(--t1)', lineHeight:1.3,
                marginTop: idx===0 ? 0 : tok.level<=2 ? 8 : 4,
                fontFamily: tok.level<=2 ? "'Syne',sans-serif" : 'inherit',
                letterSpacing: tok.level<=2 ? '-0.02em' : '-0.01em',
                paddingBottom: tok.level===1 ? 5 : 0,
                borderBottom: tok.level===1 ? '1px solid rgba(245,158,11,0.16)' : 'none',
                userSelect:'text',
              }}>
          ...
          case 'para':
            return (
              <p key={idx} style={{ margin:0, fontSize:12.5, lineHeight:1.78, color:'var(--t1)', userSelect:'text', cursor:'text', wordBreak:'break-word' }}>
                {tok.lines.map((l,j) => <span key={j}>{j>0&&<br/>}{renderInline(l)}</span>)}
              </p>
            )

          case 'ul':
            return (
              <div key={idx} style={{ display:'flex', flexDirection:'column', gap:3, margin:'1px 0' }}>
                {tok.items.map((item,j) => (
                  <div key={j} style={{ display:'flex', alignItems:'flex-start', gap:8, paddingLeft:item.indent*16, userSelect:'text' }}>
                    <span style={{ marginTop:8, flexShrink:0, width:item.indent>0?4:5, height:item.indent>0?4:5, borderRadius:'50%', background:item.indent>0?'var(--t3)':'var(--a)', opacity:item.indent>0?0.45:0.8 }}/>
                    <span style={{ fontSize:12.5, lineHeight:1.72, color:'var(--t1)', cursor:'text' }}>{renderInline(item.text)}</span>
                  </div>
                ))}
              </div>
            )
          ...
          case 'ol':
            return (
              <div key={idx} style={{ display:'flex', flexDirection:'column', gap:4, margin:'1px 0' }}>
                {tok.items.map((item,j) => (
                  <div key={j} style={{ display:'flex', alignItems:'flex-start', gap:9, userSelect:'text' }}>
                    <span style={{ minWidth:20, height:20, borderRadius:6, background:'var(--a-bg)', border:'1px solid var(--bd-a)', color:'var(--a)', fontSize:9.5, fontWeight:700, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, marginTop:1, fontFamily:"'DM Mono',monospace" }}>
                      {j+1}
                    </span>
                    <span style={{ fontSize:12.5, lineHeight:1.72, color:'var(--t1)', paddingTop:1, cursor:'text' }}>{renderInline(item)}</span>
                  </div>
                ))}
              </div>
            )
          ...
          case 'blockquote':
            return (
              <div key={idx} style={{ borderLeft:'3px solid var(--a)', paddingLeft:11, margin:'2px 0', opacity:0.75, display:'flex', flexDirection:'column', gap:2 }}>
                {tok.lines.map((l,j) => (
                  <p key={j} style={{ margin:0, fontSize:12, lineHeight:1.72, color:'var(--t2)', fontStyle:'italic', userSelect:'text' }}>
                    {renderInline(l)}
                  </p>
                ))}
              </div>
          ...
          case 'code':
            return <CodeBlock key={idx} lang={tok.lang} body={tok.body}/>

          case 'hr':
            return <div key={idx} style={{ height:1, background:'rgba(245,158,11,0.13)', margin:'3px 0' }}/>
